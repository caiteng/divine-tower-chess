...
- 备战页已改为选中驱动：点击上阵区 / 备战区单位只选中实例，右侧根据位置显示 `上阵选中` 或 `下阵选中` 小按钮，并展示选中单位信息。
...
- 备战页交互从“点卡片直接上阵 / 下阵”改为“先选中，再点右侧动作按钮”：
  - 点击备战区单位只会选中该实例。
  - 选中备战区单位时，右侧按钮显示 `上阵选中`。
  - 选中上阵区单位时，右侧按钮显示 `下阵选中`。
  - 未选中单位时，动作按钮置灰显示 `选择单位`。
- 右侧新增选中单位信息面板：
  - 显示小头像、单位名、星级、实例 ID 后缀和队长 / 神品标记。
  - 显示生命、护甲、攻击、攻击间隔、射程、移速、穿甲和碰撞体积。
  - 显示当前实例神品任务及进度；无任务时显示 `神品任务：无`。
- 备战区改成 4x2 排列，为信息面板和动作按钮留出稳定空间。
- `卖出选中` 按钮在未选中单位时置灰，避免误操作预期不清。
- 验证：`npm test` 已通过。
...
15. 上阵盾卫，确认商店 / 编队 / 战场星级展示使用新盾卫 portrait 和 `shield_guard_star1~3`。
16. 战斗中让盾卫移动、集火、承伤和倒下，确认分别播放 move、attack、idle_heavy / block、hurt、death 动作帧。
17. 战斗中让己方单位向左移动，确认贴图水平翻转后正向行走；再向右移动，确认恢复朝右。
18. 观察从左侧刷出的敌人或被推向右侧移动的敌人，确认敌人向右移动时会水平翻转，不会倒退行走。
19. 准备阶段确认备战界面完整覆盖屏幕，不露出背后的半截战场。
20. 点击开始下一波后，确认备战界面滑出，战场背景和点击区域扩展为完整战斗操作区域。
21. 上阵盾卫并让 3 个以上近战敌人接近，确认敌人会停在盾卫外沿分散攻击，不再持续贴身抖动。
22. 拉动盾卫横向走位，观察后排敌人是否会被前排敌人挡住并绕行，而不是穿过前排直接贴到盾卫。
23. 确认战场单位与敌人比之前更大，点击选择更容易。
24. 合成或载入 2 星猎人，战斗阶段选中猎人，确认底部出现 `狙击` 技能按钮。
25. 点击 `狙击`，确认技能自动锁定当前集火目标或最近敌人，产生弹道 / 伤害数字，并进入冷却倒计时。
26. 使用猎人或法师攻击敌人，在弹道出手后立刻点地面移动，确认弹道继续飞行 / 伤害正常结算，单位能马上开始移动。
27. 连续做攻击后移动操作，确认攻速冷却仍存在，不会因为移动取消而立刻再次普攻。
28. 在猎人 / 法师刚开始抬手但弹道还没出现时点地面移动，确认这次攻击被取消，不产生弹道或伤害。
29. 准备阶段点击备战区单位，确认只是选中并在右侧显示头像、属性和神品任务进度，不会立刻上阵。
30. 选中备战区单位后点击 `上阵选中`，确认单位进入上阵区；选中上阵区单位后点击 `下阵选中`，确认单位回到备战区。
31. 未选中单位时确认右侧动作按钮置灰为 `选择单位`，`卖出选中` 不可触发。
