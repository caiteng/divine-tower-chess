...
- 棋子头像统一使用固定尺寸绘制，避免不同源图比例导致的头像跑版或挤压。
- 棋盘上阵棋子在攻击触发瞬间会出现轻微抖动和劈砍光痕，增强战斗反馈。
...
- `assets/scripts/ui/cocos-game-controller.ts`
  ...
  - 固定头像 Sprite 为自定义尺寸并关闭 trim，避免头像渲染尺寸被源图裁剪影响。
  - 在战斗阶段基于攻击冷却窗口，为上阵单位增加抖动和劈砍光痕动画反馈。
...
15. 头像固定尺寸与攻击反馈
   - 在战斗中观察不同职业头像（含神品）是否保持统一显示尺寸，不再出现拉伸跑版。
   - 开战后观察上阵单位攻击瞬间，确认会出现短暂抖动与斜向劈砍光痕。
